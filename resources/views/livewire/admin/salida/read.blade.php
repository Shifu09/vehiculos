    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header p-0">
                <h3 class="card-title">{{ __('ListTitle', ['name' => __(\Illuminate\Support\Str::plural('Salida'))]) }}
                </h3>
                ...
                <div class="px-2 mt-4">
                    ...
                    <ul class="breadcrumb mt-3 py-3 px-4 rounded">
                        ...
                        <li class="breadcrumb-item active">{{ __(\Illuminate\Support\Str::plural('Salida')) }}</li>
                    </ul>

                    <div class="row justify-content-between mt-4 mb-4">
                        @if (getCrudConfig('Salida')->create && hasPermission(getRouteName() . '.salida.create', 1, 1))
                            <div class="col-md-4 right-0">
                                <a id="button" style="color: white"
                                    href="@route(getRouteName() . '.salida.create')">{{ __('CreateTitle', ['name' => __('Salida')]) }}</a>
                ...
                <table class="table table-hover table-striped">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" style='cursor: pointer' wire:click="sort('id_chofer')"> <i
                                ...
                                {{ __('Chofer') }} </th>
                            <th scope="col" style='cursor: pointer' wire:click="sort('destino')"> <i
                                    class='fa @if ($sortType == 'desc' and $sortColumn == 'destino') fa-sort-amount-down ml-2 @elseif($sortType == 'asc' and $sortColumn == 'destino') fa-sort-amount-up ml-2 @endif'></i>
                                {{ __('Destino') }} </th>
                    ...
                    <tbody>
                        @foreach ($salidas as $salida)
                            @livewire('admin.salida.single', [$salida], key($salida->id))
                        @endforeach
                    </tbody>
                </table>
            </div>
